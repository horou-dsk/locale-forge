...
impl ModelClient for OpenAiClient {
    async fn translate_batch(
        ...
        if !status.is_success() {
            ...
            let details = details.chars().take(2048).collect::<String>();
...
async fn read_limited(mut response: reqwest::Response) -> Result<Vec<u8>, ModelClientError> {
    if response
        .content_length()
    ...
    while let Some(chunk) = response
        .chunk()
        .await
        .map_err(|error| classify_transport_error("读取模型响应失败", error))?
    {
...
fn parse_response(bytes: &[u8]) -> Result<BTreeMap<String, String>, ModelClientError> {
...
fn classify_request_error(error: reqwest::Error) -> ModelClientError {
    classify_transport_error("模型请求失败", error)
}

fn classify_transport_error(context: &str, error: reqwest::Error) -> ModelClientError {
