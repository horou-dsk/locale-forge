...
```text
de: 待翻译 1057 个字段，共 6 批
de: [1/6] 第 1/3 次请求，包含 200 个字段
de: [1/6] 已完成 200/1057 个字段，本批耗时 28.4 秒
de: [2/6] 第 1/3 次请求失败（30.0 秒），250 毫秒后重试：发送模型请求超时（配置上限 30 秒）
```

若大批次频繁达到 `timeout_seconds`，应适当减小 `batch_size` 或提高超时时间，避免整批结果反复生成。
